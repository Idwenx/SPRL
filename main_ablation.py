...
LOAD_CHECKPOINT = f"{CKPT_DIR}/{args.model}_{args.k}_{args.ablation}" 
EVAL_ONLY = args.eval
target_category = -1
n_categories = 0

def run():
    print(f"Ablation: {ABLATION or 'full'}")
    ...
    if not EVAL_ONLY and ABLATION != "wo_bpr":
    ...
    if EVAL_ONLY:
        ...
        model_name = args.model if not ABLATION else f"{args.model}_{ABLATION}"
        utils.saveResults(args.dataset, cache_root="results", model_name=model_name, results=history_arr, k=K, ablation=ABLATION)
        print(f"History saved: results/{args.dataset}/{model_name}_{K}_{ABLATION}.npy  shape={history_arr.shape}")
    ...
    for episode in range(N_EPISODES):
        ...
        for t in range(env.max_steps):
            ...
            if args.model=="SPRL":
                ...
                if warmup:
                    loss, actual = model.warmup_train(graph_rep, nf, mask, env, num_traj=G)
                    status, step_reward, _, truncated, _ = env.step(actual.cpu().numpy())
                else:
                    mask_saved = mask.unsqueeze(0).clone()
                    indices, old_logp, value = model.sample_action(graph_rep, nf, mask)
                    status_prev = status.copy()
                    status, step_reward, terminated, truncated, _ = env.step(indices[0].cpu().numpy())
                    model.remember(status_prev, mask_saved, indices, old_logp, value,
                                step_reward, terminated or truncated)
                    if t == env.max_steps-1 and not warmup:
                        loss = model.update(last_status=status)
        ...
        if episode > 0 and episode % EVAL_INTERVAL == 0:
            eval_rs, _ = evaluate(env, gnn, model)
            mean_r = float(eval_rs.mean())
            if mean_r > best_eval_reward:
                best_eval_reward = mean_r
                model.save(f"{CKPT_DIR}/{args.model}_{K}_{ABLATION}")
